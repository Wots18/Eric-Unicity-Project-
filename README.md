...
This was written carefully against the published Sphere SDK docs, but the SDK is
young and method names can shift between versions. If something errors on first run:

- **Read the log line** — every network call is wrapped and logs a clear message.
- The most likely spots to need a small tweak are the **mint amount/format** and
  the **market intent** call (which is already best-effort and will simply skip if
  the API differs — DM selling still works).
- If a nametag won't register, it's taken by another wallet: pick a new `NAMETAG`.
- Copy the exact error text back to whoever is helping you build and it's usually a
  one-line fix.

Built for the Unicity "build the machine economy" call. MIT licensed — do what you like with it.
